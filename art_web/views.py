...
@app.route('/testdb')
def testdb():
    paintings = Paintings.query.with_entities(Paintings.name, Paintings.painting_photo).filter_by(artist_id=1).all()
    paintings_dict = [{'name': painting.name, 'painting_photo': painting.painting_photo} for painting in paintings]
...
def index():
    title = 'Home'
    form = FeedbackForm()
    pnt = Paintings.query.with_entities(Paintings.name, Paintings.painting_photo).filter_by(artist_id=123)
